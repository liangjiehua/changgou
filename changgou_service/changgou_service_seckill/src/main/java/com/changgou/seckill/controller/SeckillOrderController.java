...
public class SeckillOrderController {
    ...
    public Result add(Long id, String time){
        ...
        try {
            ...
            result = new Result(true, StatusCode.OK, "抢购成功！");
        } catch (RuntimeException e) {
            e.printStackTrace();
            result = new Result(false, StatusCode.ERROR, e.getMessage());
        }catch (Exception e){
            e.printStackTrace();
            result = new Result(false, StatusCode.ERROR, "系统繁忙，请稍后再试...");
        }
        return result;
    }
    ...
    public Result<SeckillStatus> queryStatus(){
        ...
        if(seckillStatus != null){
            return new Result<SeckillStatus>(true, StatusCode.OK, "当前用户有抢购订单", seckillStatus);
        }else{
            //返回20006表示没有抢购的订单
